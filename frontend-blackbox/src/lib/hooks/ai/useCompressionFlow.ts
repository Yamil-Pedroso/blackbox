...
export function useCompressionFlow(analyze: AnalyzeFunction) {
  ...
  const [currentStep, setCurrentStep] = useState<string>(compressionSteps[0]);
    ...
    return new Promise<void>((resolve) => {
      ...
      intervalRef.current = window.setInterval(() => {
        ...
        const stepIndex = Math.min(
          ...
          Math.floor((nextProgress / 100) * compressionSteps.length),
        );
  ...
  const startCompression = useCallback(
    async ({ request, targetPrecision, modelName }: CompressionRequest) => {
