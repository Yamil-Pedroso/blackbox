...
function answerFromContext(question: string, context: string) {
  const questionWords = new Set(
    wordsFrom(question).filter((word) => word.length > 3),
  );
  ...
  for (const sentence of sentences) {
    const score = wordsFrom(sentence).filter((word) =>
    ...
    return {
      ...
      score:
        label === "NEUTRAL"
          ? 0.5
          : clamp(
              0.65 + Math.abs(positiveHits - negativeHits) * 0.1,
              0.65,
              0.99,
            ),
...
class HuggingFacePipelinesProvider extends LocalPipelinesProvider {
  ...
  private async callModel<T>(model: string, inputs: unknown) {
    if (!env.hf_api_key) {
      throw new Error(
        "HF_API_KEY is required when PIPELINES_PROVIDER=hugging-face",
      );
    }
  ...
  async questionAnswering(
    ...
    return this.callModel<QuestionAnsweringResult>(
  ...
  async classify(dto: ClassificationDto): Promise<ClassificationResult> {
    ...
    >(this.models.classification, dto.text);
...
function createProvider(): PipelinesProvider {
  const provider = process.env.PIPELINES_PROVIDER as
    | PipelineProviderName
    | undefined;
...
export function generateText(dto: TextGenerationDto) {
  return withMetadata("textGeneration", (provider) =>
    provider.generateText(dto),
  );
}

export function generateAudio(dto: AudioGenerationDto) {
  return withMetadata("audioGeneration", (provider) =>
    provider.generateAudio(dto),
  );
}
