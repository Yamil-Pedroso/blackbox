...
async function loadAiSdk() {
  const [ai, { openai }] = await Promise.all([
    import("ai"),
...
export async function chatController(req: Request, res: Response) {
  try {
    const parsedBody = uiMessagesSchema.safeParse(req.body);

    if (!parsedBody.success) {
      return res.status(400).json({
        message:
          "Invalid body. Send AI SDK UI messages with id, role and parts.",
        example: {
          messages: [
            {
              id: "message-1",
              role: "user",
              parts: [{ type: "text", text: "Explain streaming responses." }],
            },
          ],
        },
      });
    ...
    const { ai, model } = await loadAiSdk();
    ...
    const result = ai.streamText({
      ...
      messages: await ai.convertToModelMessages(
        messages as Parameters<typeof ai.convertToModelMessages>[0],
      ),
    });
          ...
          messages: [
            {
              ...
              parts: [
                {
                  type: "text",
                  text: "What should I plan for a day in Zurich?",
                },
              ],
            },
          ],
...
export async function speechController(req: Request, res: Response) {
  try {
    const { ai, speechModel } = await loadAiSdk();
