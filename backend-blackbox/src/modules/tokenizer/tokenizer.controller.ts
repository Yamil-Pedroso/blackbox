...
export async function tokenizeController(req: Request, res: Response) {
  try {
    const input = validateTokenizeInput(req.body);
    return res.status(200).json(await tokenizeText(input));
  } catch (error) {
...
export async function analyzeTokenizerController(req: Request, res: Response) {
  try {
    const input = validateAnalyzeTokenizerInput(req.body);
    return res.status(200).json(await analyzeText(input));
  } catch (error) {
    if (error instanceof TokenizerValidationError) {
      return res.status(400).json({
